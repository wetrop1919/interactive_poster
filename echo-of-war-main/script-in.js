...
async function loadClues() {
	try {
		...
		stepText.textContent =
			'Невозможно загрузить подсказки. Убедитесь, что сайт запущен через http(s) и файл data/clues.json доступен.'
		return
	}
	...
	if (!Array.isArray(clues) || clues.length === 0) {
		...
		stepText.textContent =
			'Файл data/clues.json пуст или содержит неверную структуру.'
		return
	}
	...
	if (raw) {
		try {
			...
			console.warn('Неверный формат savedState', e)
		}
	}
	...
	if (savedState.completed && savedState.completed.includes(startIdx)) {
		const next = clues.findIndex(
			(c, idx) => !savedState.completed.includes(idx)
		)
...
function generateHotspotsRandomized() {
	...
	for (let i = 0; i < clues.length; i++) {
		...
		if (savedState.completed && savedState.completed.includes(i))
...
function handleClick(clickedIndex, btnElement) {
	...
	viewerTitle.textContent =
		clicked.objectName || clicked.title || `Объект ${clickedIndex + 1}`
	// подробный ответ показывается только в модальном окне (viewerDesc)
	viewerDesc.textContent =
		clicked.descr || clicked.objectDesc || clicked.clue || ''
...
function markCompleted(index, btnElement) {
	const btn =
		btnElement || document.querySelector(`.hotspot[data-index="${index}"]`)
...
function advanceStep() {
	...
	const nextIndexAfter = clues.findIndex(
		(c, idx) => !savedState.completed.includes(idx) && idx > currentIndex
	)
	if (nextIndexAfter !== -1) {
...
function resetProgress() {
	...
	saveState()
	document
		.querySelectorAll('.hotspot.completed')
		.forEach(n => n.classList.remove('completed'))
...
function viewDiv() {
	window.scrollTo(0, 0)
	const el = document.getElementById('theEnd')
	if (el) el.style.display = 'block'
}
const backBtn = document.getElementById('back-button')

backBtn.addEventListener('click', () => {
	const endDiv = document.getElementById('theEnd')
	if (endDiv) endDiv.style.display = 'none'

	// Вернуться к последнему шагу
	currentIndex = clues.length - 1
	savedState.currentIndex = currentIndex
	saveState()
	renderStep()
})
...
skipBtn.addEventListener('click', () => {
	// Пометить все шаги как завершённые
	savedState.completed = clues.map((_, idx) => idx)
	completedCounter.textContent = clues.length
	currentIndex = clues.length - 1
	savedState.currentIndex = currentIndex
	saveState()
	renderStep()
	viewDiv() // ← вызываем вручную для проверки
})*/
